...
class DirectoryBlock extends Block {
      ...
      public DirectoryBlock(int bNo)
      {
         ...
         hardLink = new boolean[NUM_ENTRIES];
      }
      ...
      public boolean addEntry(String name, int iNo){
         return addEntry(name, iNo, false);        
      }                         
      public boolean addEntry(String name, int iNo, boolean isLink)
      {
         if (numEntries < NUM_ENTRIES)
         {
             inodeNo[numEntries] = iNo;
             this.name[numEntries] = name;
             hardLink[numEntries] = isLink;
             numEntries++;
             return true;
         }  
         else return false;
      ...
      public boolean isHardLink(String name)
      {
         for(int i=0; i<numEntries; i++)
            if (this.name[i].equals(name))
               return hardLink[i];
