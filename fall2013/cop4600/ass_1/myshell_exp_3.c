		     ...
		     if(fork == 0){ //end fork
		     	/*if(i == numPipes-2){
			    	...
			    	perror("execvp failed");
		      		...
		      		printf("child3\n");
		     		dup2(fd[1][0], STDIN_FILENO); //std_in comes from current pipe
			      	//close(fd[1][0]);
			      	close(fd[1][1]);
			      	close(fd[0][0]);
			      	close(fd[0][1]);
		     	...
		     	/*if(i == numPipes-2){
			    	...
			    	perror("execvp failed");
		      		...
		      		printf("child2\n");
		     		dup2(fd[0][0], STDIN_FILENO); //std_in comes from current pipe
			      	dup2(fd[1][1], STDOUT_FILENO); //std_out goes to next pipe
			      	close(fd[1][0]);
			      	//close(fd[1][1]);
			      	//close(fd[0][0]);
			      	close(fd[0][1]);
