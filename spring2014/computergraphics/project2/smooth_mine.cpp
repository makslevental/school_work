...
double framebuffer[IMAGE_WIDTH][IMAGE_HEIGHT][6];
...
void bresenham(int x0, int y0, double z0, int x1, int y1, double z1, GLubyte* color){
    int dx = abs(x1-x0);
    int dy = abs(y1-y0);
    int sx = x0 < x1 ? 1 : -1;
    ...
    while(true){
        ...
        //scaled = 1;
        framebuffer[y][x][2] = (double(color[0])*scaled);
        framebuffer[y][x][3] = (double(color[1])*scaled);
        framebuffer[y][x][4] = (double(color[2])*scaled);
...
//model is global
void plotTriangleToFrameBuffer(double vertices[3][3], GLubyte* color){

    bresenham(vertices[0][0],vertices[0][1], vertices[0][2], vertices[1][0], vertices[1][1], vertices[1][2], color);
    bresenham(vertices[1][0],vertices[1][1], vertices[1][2], vertices[2][0], vertices[2][1], vertices[2][2], color);
    bresenham(vertices[2][0],vertices[2][1], vertices[2][2], vertices[0][0], vertices[0][1], vertices[0][2], color);
    printf("\n");
}

void scanlineTriangleToFrameBuffer(double vertices[3][3], GLubyte* color){
    ...
    for(int y = miny-1; y < maxy+1; y++){
        ...
        if(crossing == 4){
                ...
                for(int k = crossings[1][1]; k < crossings[2][1]; k++){
                    ...
                    //scaled = 1;
                    framebuffer[y][k][2] = (double(color[0])*scaled);
                    framebuffer[y][k][3] = (double(color[1])*scaled);
                    framebuffer[y][k][4] = (double(color[2])*scaled);
...
//    for(int i = 0; i < model->numtriangles; i++){
    int j = 1;
    for(int i = j; i < j+4/*model->numtriangles*/; i++){
    ...
    //        std::vector<double> vertx(myverts, myverts + sizeof(myverts) / sizeof(double) );
        ...
        plotTriangleToFrameBuffer(vertices, black);
        scanlineTriangleToFrameBuffer(vertices, black);
