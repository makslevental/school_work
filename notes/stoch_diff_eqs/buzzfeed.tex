...
\lyxframeend{}\subsection{Baby steps}


\lyxframeend{}\subsection{Martingale}
...
such that every finite dimensional joint distribution is specified}%
\footnote{And they obey some regularity conditions cf. Kolmogorov Extension
thm.%
}{\footnotesize{}. For fixed $\omega$, $X_{t}\left(\omega\right)$
is a }\emph{\footnotesize{}sample path}{\footnotesize{} or }\emph{\footnotesize{}realization
}{\footnotesize{}.}{\footnotesize \par}\end{block}
\begin{itemize}
...
- De Moivre%
\footnote{{\tiny{}$X_{i}\sim$ Bernoulli$\left(p\right)\Rightarrow\lim_{n\rightarrow\infty}P(a\leq\sum X_{i}-np/\sqrt{np\left(1-p\right)})\leq b=\left(2\pi\right)^{-1/2}\int_{a}^{b}e^{-\frac{x^{2}}{2}}dx$}%
} we need 
\begin{eqnarray*}
...
and finally 
\begin{eqnarray*}
\lim_{\underset{\underset{,\Delta tD=\left(\Delta x\right)^{2}}{t=n\Delta t}}{n\rightarrow\infty}}P\left(a\leq\sqrt{Dt}X\left(t\right)\leq b\right) & = & \lim_{\underset{\underset{,\Delta tD=\left(\Delta x\right)^{2}}{t=n\Delta t}}{n\rightarrow\infty}}P\left(\sqrt{Dt}a\leq X\left(t\right)\leq\sqrt{Dt}b\right)\\
...
\begin{itemize}
\item $X_{\omega}\left(t\right)\triangleq X\left(\omega_{0},t\right)$ as
a sample path is a continuous function from $\mathbb{R}^{+}\rightarrow\mathbb{R}$
...
\begin{equation}
LQ''\left(t\right)+RQ'\left(t\right)+\frac{1}{C}Q\left(t\right)=F\left(t\right),\; Q\left(0\right)=Q_{0},\; Q'\left(0\right)=I_{0}\label{eq:lrc-1}
\end{equation}
If $F\left(t\right)=\cos\left(\omega t\right)$ and $F\left(0\right)=F_{0}$
the solution is simple: 
\[
Q\left(t\right)=c_{1}e^{r_{1}t}+c_{2}e^{r_{2}t}+A\sin\left(\omega t-\phi\right)
\]
for some constants%
\footnote{Each are constant functions of boundary values \textrm{$Q_{0},I_{0},F_{0}$.}%
} $c_{1},r_{1},c_{2},r_{2},A,\phi$. But what if $F\left(t\right)=G\left(t\right)+\text{"noise"}$?
\item Consider taking noisy measurements $Z\left(t\right)$ where 
\[
Z\left(t\right)=Q\left(t\right)+\text{"noise"}
\]



What is the best estimate of $Q\left(t\right)$ satisfying eqn \ref{eq:lrc-1}
based on $Z\left(t\right)$? Kalman Filter.
...
process $W_{t}$ and so 
\[
\frac{dX_{t}}{dt}=b\left(t,X_{t}\right)+\sigma\left(t,X_{t}\right)\cdot W_{t}
\]
...
\end{enumerate}
\end{theorem}%{}
Property 3 is related to the fact that Ito integrals are martingales.
\begin{definition}%{}
...
Does 
\[
\int_{0}^{T}B\left(t\right)dB_{t}=\frac{1}{2}\left(B\left(t\right)\right)^{2}
\]
If this were regular calculus it would but we're not in Kansas anymore;
using Ito's formula with $Y\left(t\right)=\frac{1}{2}\left(B\left(t\right)\right)^{2}$
we get that 
\begin{eqnarray*}
d\left(Y\left(t\right)\right) & = & d\left(\frac{1}{2}\left(B\left(t\right)\right)^{2}\right)=\frac{\partial g}{\partial t}dt+\frac{\partial g}{\partial x}dB_{t}+\frac{1}{2}\frac{\partial^{2}g}{\partial x^{2}}\left(dB_{t}\right)^{2}\\
 & = & B\left(t\right)dB_{t}+\frac{1}{2}\left(dB_{t}\right)^{2}=B\left(t\right)dB_{t}+\frac{1}{2}dt
\end{eqnarray*}
and so
\[
\int d\left(\frac{1}{2}\left(B\left(t\right)\right)^{2}\right)=\int_{0}^{T}B\left(t\right)dB_{t}+\int_{0}^{T}\frac{1}{2}dt
\]
implies
\[
\int_{0}^{T}B\left(t\right)dB_{t}=\frac{1}{2}\left(\left(B\left(t\right)\right)^{2}-T\right)
\]
...
the asset, $r$ is the risk-free interest rate, and $\sigma$ is the
...
First for $I_{n}\left(s,\omega\right)$ with $\phi_{n}\left(\omega,t\right)=\sum_{j}a_{j}^{\left(n\right)}\left(\omega\right)\mathbf{1}_{\left[t_{j}^{\left(n\right)},t_{j+1}^{\left(n\right)}\right)}\left(t\right)$:
\begin{eqnarray*}
E\left(I_{n}\left(\omega,s\right)\big|\mathcal{F}_{t}\right) & = & E\left(\int_{0}^{t}\phi_{n}dB+\int_{s}^{t}\phi_{n}dB\big|\mathcal{F}_{t}\right)\\
 & = & \int_{0}^{t}\phi_{n}dB+E\left(\sum_{t\leq t_{j}^{\left(n\right)}\leq t_{j+1}^{\left(n\right)}\leq s}a_{j}^{\left(n\right)}\Delta B_{j}\Bigg|\mathcal{F}_{t}\right)\\
 & = & \int_{0}^{t}\phi_{n}dB+\sum_{j}E\left(E\left(a_{j}^{\left(n\right)}\Delta B_{j}\Bigg|\mathcal{F}_{t_{j}^{\left(n\right)}}\right)\Bigg|\mathcal{F}_{t}\right)\\
 & = & \int_{0}^{t}\phi_{n}dB+\sum_{j}E\left(a_{j}^{\left(n\right)}\cancel{E\left(\Delta B_{j}\Bigg|\mathcal{F}_{t_{j}^{\left(n\right)}}\right)}\Bigg|\mathcal{F}_{t}\right)\\
 & = & I_{n}\left(\omega,t\right)
\end{eqnarray*}
...
Then by convergence of a.s convergence of $I_{n}\left(\omega,t\right)\rightarrow I\left(\omega,t\right)$
we get that $I\left(\omega,t\right)$ is a martingale.
