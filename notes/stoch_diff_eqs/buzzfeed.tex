...
\section{S. Differential Equations}


\section{Appendix}


\subsection{Laplace-De Moivre}


\subsection{Differentiable nowhere}
...
path}{\footnotesize{} or }\emph{\footnotesize{}trajectory}{\footnotesize{}.}{\footnotesize \par}\end{block}
\begin{itemize}
...
\item {\footnotesize{}Hiddern Markov models (hierarchical model) great for
...
\]
is the position of the particle at time $n\Delta t$. To use Laplace - De moivre

\footnote{\tiny{}$X_{i}\sim$ Bernoulli$\left(p\right)\Rightarrow\lim_{n\rightarrow\infty}P(a\leq/\sqrt{np^{2}}\leq b)$ $=\frac{1}{\sqrt{2\pi}}\int_{a}^{b}e^{-\frac{x^{2}}{2}dx}$
} 
...
Then
\begin{eqnarray*}
X\left(n\Delta t\right) & = & \sqrt{Dt}\left[\left(\frac{\frac{1}{n}\left(\sum_{i=1}^{n}X_{i}\right)-\frac{1}{2}}{\sqrt{1/4}/\sqrt{n}}\right)\right]
\end{eqnarray*}
...
and finally 
\begin{eqnarray*}
\lim_{n\rightarrow\infty}P\left(a\leq\sqrt{Dt}X\left(t\right)\leq b\right) & = & \lim_{n\rightarrow\infty}P\left(a\leq\sqrt{Dt}X\left(t\right)\leq b\right)\\
...
the white noise space and $\mathbf{L}_{2}\left(\Omega\right)\triangleq\mathbf{L}_{2}\left(\Omega,\mathcal{B}\left(\mathscr{S}_{\mathbb{R}}^{'}\right),P\right)$.
...
\end{eqnarray*}
Know $X_{s}$ up to present $t$ - when to sell? Since noisey \emph{optimal
stopping strategy} maximizes expected returns. Can be solved by solving
a corresponding semielliptic second order PDE with Dirichlet boundary
conditions.
\end{itemize}
