...
\begin{document}
    \begin{tikzpicture}
        \node[draw,circle,minimum size=80pt] (x) at (1,0) {$\sigma\left(w_0 + \sum\limits_{i=1}^{n}{w_ix_i}\right)$};
        ...
        % 	\draw[stateTransition] (x) -- (4,-1) node [midway,above] {$\sigma\left(w_0 + \sum\limits_{i=1}^{n}{w_ix_i}\right)$};
